...
import {AssetManager, AssetLoader, Texture, Canvas, ObjectEx} from "phina.js";
...
export class TiledMap extends XMLLoader {
  ...
  _cloneObjectLayer(srcLayer) {
    const result = ObjectEx.$safe.call({}, srcLayer);
    result.objects = [];
    //レイヤー内オブジェクトのコピー
    srcLayer.objects.forEach(obj => {
      const resObj = {
        properties: ObjectEx.$extend.call(obj, ObjectEx.$safe.call({}, obj.properties))
      };
  ...
  //ローダーに追加
  static register() {
    AssetLoader.register("tmx", function(key, path) {
